pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("krisnameera123/react-app")
                    app.inside {
        ...
        stage('Push Docker Image') {
            ...
            steps {
                script {
                        docker.withRegistry('https://registry-1.docker.io/v2/', 'dockerhub_login')  {
         ...
         stage('DeployToStaging') {
            ...
            steps {
                    script {
                        sh "docker pull krisnameera123/react-app:${env.BUILD_NUMBER}"
                        try {
                        ...
                        sh "docker run --restart always --name react-app -p 1233:80 -d krisnameera123/react-app:${env.BUILD_NUMBER}"
                    }
            }
        }
